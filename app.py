...
st.set_page_config(
    page_title="BloggerAgent AI",
    page_icon="✍️",
    layout="wide"
)

# API Initialization
api_key = os.getenv("GEMINI_API_KEY") or "AQ.Ab8RN6L3-VU72AUfOx3OcQWPCTWasC33h_g3ag4rZu-Fsy20sg"
...
# Main Chat Bar
user_prompt = st.chat_input("Ask anything... (e.g. Write a tech blog, recipe breakdown, code explanation)")

has_voice_only = voice_audio is not None and not user_prompt
active_input = user_prompt if user_prompt else ("Voice Note Query" if has_voice_only else None)
        ...
        with st.spinner("Processing & writing..."):
            try:
                ...
                if has_voice_only and user_audio_bytes:
                    payload.append(types.Part.from_bytes(data=user_audio_bytes, mime_type="audio/wav"))
                    payload.append("Transcribe and provide a comprehensive, structured response in natural conversational language.")
                else:
            ...
            if HAS_GTTS and not ai_text.startswith("⚠️"):
                try:
                    clean_text = ai_text.replace("*", "").replace("#", "").replace("`", "")[:450]
